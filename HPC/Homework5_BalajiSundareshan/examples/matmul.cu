...
int main(int argc, char const *argv[])
{
    ...
    printf("Elapsed time for matrix multiplication of a %d x %d matrix by a  %d x %d matrix on a GPU: %f ms.\n\n", m, n, n, k, gpu_elapsed_time_ms);
    ...
    int all_ok = 1;
    printf("Elapsed time for matrix multiplication of a %d x %d matrix by a  %d x %d matrix on a CPU: %f ms.\n\n", m, n, n, k, cpu_elapsed_time_ms);
    for (int i = 0; i < m; ++i)
    {
        for (int j = 0; j < k; ++j)
        {
