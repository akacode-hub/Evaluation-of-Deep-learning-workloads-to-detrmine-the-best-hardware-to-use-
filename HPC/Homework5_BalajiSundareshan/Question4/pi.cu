...
__global__ void flagKernel(unsigned *block_counts) {
  ...
  // printf("blockidx x: %d, blockDim x: %d, blockidx y: %d, blockDim y: %d, threadIdx x: %d, threadIdx y: %d, gridDim x: %d, gridDim y: %d\n",blockIdx.x, blockDim.x, blockIdx.y, blockDim.y, threadIdx.x, threadIdx.y, gridDim.x, gridDim.y);
...
int main() {
  ...
  size_t N = BLOCK_DIM*100;    // grid size
